...
class Baniry_search_tree:
    ...
    def _BFS_list(self, node, i):#层级遍历,广度优先搜索
        if node.left != None:
            self.nodes.append(node.left)
        if node.right != None:
            self.nodes.append(node.right)
        if i < len(self.nodes)-1:
            self._BFS_list(self.nodes[i+1], i+1)
...
def test(tree):
    tree.delete(0)#删除一个节点
    
    tree.list_nodes()#遍历
    
    node = Node(data = 888, name = 20)
    tree.add(node)#增加一个节点
